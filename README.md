...
Это гарантирует сохранение данных при перезапуске контейнера.

---

## Безопасность

- **Нет SQL-базы** — SQL-инъекции невозможны
- **XSS** — все данные выводятся через `textContent` (не `innerHTML`)
- **SSRF** — домен запросов жёстко ограничен `music.yandex.ru`
- **Бот-скан** — запросы вроде `/stripe.json`, `/.env` возвращают 404 и не представляют угрозы
...
- [ ] OAuth авторизация через Spotify (для создания плейлистов на аккаунте пользователя)
